...
function deriveInsights(summary: AccountSummaryData, accounts: UnifiedAccount[]): Insight[] {
  ...
  if (highUsageCc.length > 0) {
    const names = highUsageCc.map(a => a.name.split(' ')[0]).join(', ')
    insights.push({
      id: 'high-cc',
      emoji: '⚠️',
  ...
  // Banyak akun idle
  const idleAccounts = accounts.filter(a => a.type !== 'credit' && (a.balance ?? 0) === 0)
