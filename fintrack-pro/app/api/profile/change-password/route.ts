 ...
 * Changes an existing password. Works for ALL users who have a password set
 * (including Google users who previously used /api/profile/set-password).
 */
export async function POST(request: Request) {
  ...
  try {
    ...
    // auth node might not exist for pure Google users who never set a password
    if (!authSnap.exists())
      return NextResponse.json({ success: false, error: 'Akun ini belum memiliki password. Gunakan Set Password.' }, { status: 400 })
