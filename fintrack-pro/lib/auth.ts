  ...
  try {
    ...
    if (!snap.exists()) {
      await ref.set({
        ...
        image:           data.image  ?? '',
        provider:        'google',
        profileCompleted: false,   // set true after username+password created
  ...
  callbacks: {
    async jwt({ token, user, account, trigger }) {
      ...
      if (account?.provider === 'google' && account.providerAccountId) {
        ...
        // Check if profile completed (done async — read from DB)
        // Use a lightweight flag stored on the token
    ...
    async signIn({ user, account }) {
      if (account?.provider === 'google') {
        ...
        // Check if this Google user already has credentials set
        // We store this info in profile.profileCompleted
        // The redirect to /complete-profile is handled by middleware/dashboard
      }
      return true
    },
  },
